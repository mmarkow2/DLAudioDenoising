...
Another widely used method is deep recurrent neural networks\cite{valentini2016speech} which have good performance on extracting acoustic features from noisy data. It use a parallel database of noisy and clean acoustics parameters as input and output. But it have a high computation cost.
...
So we set $\lambda = 0.025$ to test different atoms.
...
After we examine the output, the $15 \times \text{atoms}$ turned to have a better precise.

Our experiments have showed that online dictionary learning is very efficient and robust on audio denoising problem.
